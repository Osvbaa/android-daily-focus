...
class OfflineFirstTaskRepository @Inject constructor(
    ...
    override suspend fun saveTask(id: TaskId?, draft: TaskDraft, expectedRevision: Long?): TaskMutationResult = mutate {
        draft.projectId?.let { projectId ->
            check(projectDao.getProject(projectId.value) != null) { "Project not found" }
        }
        draft.milestoneId?.let { milestoneId ->
            val milestone = checkNotNull(projectDao.getMilestone(milestoneId.value)) { "Milestone not found" }
            check(draft.projectId?.value == milestone.projectId) { "Milestone belongs to another project" }
        }
    ...
    override suspend fun createTaskFromNote(title: String, noteId: String): TaskMutationResult = mutate {
        require(title.isNotBlank()) { "Task title cannot be blank" }
        val task = Task(
            id = TaskId(idGenerator.nextId()),
            title = title.trim(),
            createdAtMillis = wallClock.currentTimeMillis(),
        )
        taskDao.insertTaskWithNoteLink(task.asEntity(), noteId)
        TaskMutationResult.Success(task.id)
    }
